...
if __name__ == "__main__":
    ...
    if args.opt == 'mpo_pioglitazone':
        ...
        col = ['SMILES', 'mol_id', 'disimilarity', 'mw', 'rb', 'sim','CV']
    ...
    simbank = pd.read_csv('./data/archive/guacamol_multibank_200.csv').values
    ...
    #a = list(range(0, 81, 20))  # 180
    #b = list(range(20, 101, 20))  # 200
    a = [0]
    b = [1]
    c = 2  # 几条约束，计算约束违反度
    ...
    for num in range(len(a)):
        mm1 = a[num]
        mm2 = b[num]
        pop_smis_iter = [['SMILES', 'mol_id', 'iter',  'dis_sim', 'mw','rb','sim','CV']]  # 分批存储每一代进化属性值，CV！！！
        archive_smis_iter = [['SMILES', 'mol_id', 'iter','dis_sim', 'mw','rb','sim','CV']]
            ...
            while r <= restart:
                ...
                while iter < nIter:
                    ...
                    if iter < nIter_1:
                        ...
                        try:
                            dominated_hypervolume = pg.hypervolume(np.array([-1.0 * fit for fit in fits_c if
                                                                             (np.array(fit) >= [0, 0, 0, 0]).all()])).compute(
                                np.zeros(len(fits[0])))
                        except:
                        ...
                        try:
                            dominated_hypervolume = pg.hypervolume(np.array([-1.0 * fit for fit in archive_fits_c if
                                                                             (np.array(fit) >= [0, 0, 0, 0]).all()])).compute(
                                np.zeros(len(fits[0])))
                        except:
                            dominated_hypervolume = 0
