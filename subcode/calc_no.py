...
def check_energy(lig_): 
    ...
    try:
        ...
        command_obabel_check = subprocess.run(ob_cmd, stdout=subprocess.PIPE)
...
def run_docking_1syh(lig_location, out_location, method='qvina', path=None):
    """
    ...
    """
    if method == 'qvina': 
        command_run = subprocess.run([f"{path}/data/qvina", "--receptor", f"{path}/docking_structures/1syh/prot.pdbqt", "--ligand", lig_location, "--center_x", "21.492800140380858", "--center_y", "13.457733376820881", "--center_z", "23.175899950663247", "--size_x", "20", "--size_y", "20", "--size_z", "20", "--exhaustiveness", "10", "--out", out_location], stdout=subprocess.PIPE)
    elif method == 'smina': 
        command_run = subprocess.run([f"{path}/data/smina", "--receptor", f"{path}/docking_structures/1syh/prot.pdbqt", "--ligand", lig_location, "--center_x", "21.492800140380858", "--center_y", "13.457733376820881", "--center_z", "23.175899950663247", "--size_x", "20", "--size_y", "20", "--size_z", "20", "--exhaustiveness", "100", "--out", out_location], stdout=subprocess.PIPE)
    else: 
...
def run_docking_4lde(lig_location, out_location, method='qvina', path=None):
    """
    ...
    """
    if method == 'qvina': 
        command_run = subprocess.run([f"{path}/data/qvina", "--receptor", f"{path}/docking_structures/4lde/prot.pdbqt", "--ligand", lig_location, "--center_x", "-2.942962976793448", "--center_y", "-12.915592617458767", "--center_z", "-50.99233344749168", "--size_x", "20", "--size_y", "20", "--size_z", "20", "--exhaustiveness", "10", "--out", out_location], stdout=subprocess.PIPE)
    elif method == 'smina': 
        command_run = subprocess.run([f"{path}/data/smina", "--receptor", f"{path}/docking_structures/4lde/prot.pdbqt", "--ligand", lig_location, "--center_x", "-2.942962976793448", "--center_y", "-12.915592617458767", "--center_z", "-50.99233344749168", "--size_x", "20", "--size_y", "20", "--size_z", "20", "--exhaustiveness", "100", "--out", out_location], stdout=subprocess.PIPE)#capture_output=True
    else: 
        raise Exception('Possible docking softwares: qvina/smina')
...
def run_docking_6y2f(lig_location, out_location, method='qvina', path=None):
    """
    ...
    """
    if method == 'qvina': 
        command_run = subprocess.run([f"{path}/data/qvina", "--receptor", f"{path}/docking_structures/6y2f/prot.pdbqt", "--ligand", lig_location, "--center_x", "11.026168696851615", "--center_y", "-0.6082891440804464", "--center_z", "20.840999947973046", "--size_x", "10", "--size_y", "10", "--size_z", "10", "--exhaustiveness", "10", "--out", out_location], stdout=subprocess.PIPE)
    elif method == 'smina': 
        command_run = subprocess.run([f"{path}/data/smina", "--receptor", f"{path}/docking_structures/6y2f/prot.pdbqt", "--ligand", lig_location, "--center_x", "11.026168696851615", "--center_y", "-0.6082891440804464", "--center_z", "20.840999947973046", "--size_x", "10", "--size_y", "10", "--size_z", "10", "--exhaustiveness", "100", "--out", out_location], stdout=subprocess.PIPE)
    else: 
        raise Exception('Possible docking softwares: qvina/smina')
...
def perform_calc_single(smi, receptor_type, docking_program='qvina', scratch='/tmp'):
    """
    ...
    try:
        ...
        if lig_energy < 10000:
            if receptor_type == '1syh':
                score_ = run_docking_1syh(lig_location, out_location, method=docking_program, path=up_path)
            if receptor_type == '4lde':
                score_ = run_docking_4lde(lig_location, out_location, method=docking_program, path=up_path)
            if receptor_type == '6y2f':
                score_ = run_docking_6y2f(lig_location, out_location, method=docking_program, path=up_path)
        else:
            os.chdir(owd)
            tmp_dir.cleanup()
            return 10**4
