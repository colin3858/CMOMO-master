...
if __name__ == "__main__":
    ...
    elif args.opt == 'qedsajnk':
        ff = ff_qedjnksa
        fitness = fitness_qedjnksa
        tres = [0.8, 0.5, 0.7, 0.3]
        col = ['SMILES', 'mol_id', 'qed', 'jnk3', 'sa_nom', 'sim']
    elif args.opt == 'qedsagsk':
        ff = ff_qedgsksa
        fitness = fitness_qedgsksa
        tres = [0.8, 0.5, 0.7, 0.3]
        col = ['SMILES', 'mol_id', 'qed', 'gskb', 'sa_nom', 'sim']
    elif args.opt == 'qedsadrd':
        ff = ff_qeddrdsa
        fitness = fitness_qeddrdsa
        tres = [0.8, 0.5, 0.7, 0.3]
        col = ['SMILES', 'mol_id', 'qed', 'drd2', 'sa_nom', 'sim']
    args = parser.parse_args()
    # device = 'cuda'
    ...
    for num in range(len(a)):
        mm1 = a[num]
        mm2 = b[num]
        pop_smis_iter = [['SMILES', 'mol_id', 'iter', 'QED', 'sim', 'pogp_imp','CV']]  # 分批存储每一代进化属性值，CV
            ...
            while r <= restart:
                ...
                while iter < nIter:
                    ...
                    if iter < nIter_1:
                        ...
                        ###save cHV, cPD in stage 1,先找出可行解属性
