...
if __name__ == "__main__":
    ...
    parser.add_argument("--opt", default='4lde',
    ...
    model = CDDDModel()
    # canonicalize
    data = pd.read_csv('./data/test_4lde_500.csv').values

    simbank = pd.read_csv('./data/archive/docking_multibank_100.csv').values
    ...
    for num in range(len(a)):
        mm1 = a[num]
        mm2 = b[num]
        pop_smis_iter = [['SMILES', 'mol_id','iter', 'qed', 'sim', '4lde','CV']]  # 分批存储每一代进化属性值，CV！！！
        archive_smis_iter = [['SMILES', 'mol_id','iter', 'qed', 'sim', '4lde','CV']]
                ...
                while iter < nIter:
                    ...
                    if iter < nIter_1:
                        ...
                        for i in range(len(smis)):
                            tuple = [smis[i], nn, iter, fits[i][0], fits[i][1], fits[i][2], CV_pops[i]]
                            pop_smis_iter.append(tuple)
                        ### save smiles in archive at stage1
                        for i in range(len(Archive_smi)):
                            tuple = [Archive_smi[i], nn, iter, Archive_fits[i][0], Archive_fits[i][1],Archive_fits[i][2], Archive_CV[i]]
                            archive_smis_iter.append(tuple)
                        ###save HV, PD in stage 1
                        fits_c = fits[np.where(np.array(CV_pops) == 0)]
                        try:
                            dominated_hypervolume = pg.hypervolume(
                                np.array([[-1.0, -1, -0.1] * np.array(fit) for fit in fits_c if
                                          (np.array(fit) > [0, 0, 0]).all()])).compute(
                                np.zeros(len(fits[0])))
                        except:
                        ...
                        try:
                            dominated_hypervolume = pg.hypervolume(
                                np.array([[-1.0, -1, -0.1] * np.array(fit) for fit in archive_fits_c if
                                          (np.array(fit) > [0, 0, 0]).all()])).compute(
                                np.zeros(len(fits[0])))
                        except:
                            dominated_hypervolume = 0
                        ...
                        chrpops = crossover(Archive_pops, pc, d, lb, rb)  # 混合线性交叉
                        chrpops = group_mut(chrpops,lb, rb, pm, numberOfGroups)
                        # 评估子代属性值
                        chrpops = torch.from_numpy(chrpops)
                        chrmol, chrsmiles = model.decode(chrpops)  ###解码潜在表示，分子和SMILES
                ...
                for i in range(len(paretoendsmiles)):
                    if paretoCV[i] == 0:
                        if paretoendsmiles[i] not in unique_smiles:
                            ...
                            tuple = (paretoendsmiles[i], nn, paretoFits[i][0], paretoFits[i][1], paretoFits[i][2], paretoCV[i])
                            smi_iter_last.append(tuple)
                            if (np.array(paretoFits[:][i]) >= tres).all() == 1:
                                rr.append(1)
